...
func (s *Session) schedule() (ctx *protocol.FrameCtx) {
	...
	case protocol.SFrame:
		...
		if s.handler != nil {
			ctx = s.frameHelper()
			s.handler.ReceivedSFrameHandle(s.peerCode, s, lsb, msb, ctx)
		}
	case protocol.UFrame:
		...
		if s.handler != nil {
			s.handler.ReceivedUFrameHandle(s.peerCode, s, param, ctx)
		}
	default:
		return
	}
